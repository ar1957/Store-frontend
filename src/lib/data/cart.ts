 ...
 */
export async function retrieveCart(cartId?: string, fields?: string, cacheOverride?: RequestCache) {
  ...
  const cacheStrategy = cacheOverride ?? (fields?.includes("payment_collection") ? "no-store" : "force-cache")
...
/**
 * Add this function to src/lib/data/cart.ts
 * It saves the address without redirecting — safe for single-page checkout
 */
export async function saveShippingAddress(formData: FormData): Promise<string | null> {
  try {
    const cartId = await getCartId()
    ...
    const data = {
      shipping_address: {
        ...
        last_name: formData.get("shipping_address.last_name"),
        address_1: formData.get("shipping_address.address_1"),
        address_2: formData.get("shipping_address.address_2") || null,
        postal_code: formData.get("shipping_address.postal_code"),
        city: formData.get("shipping_address.city"),
        country_code: formData.get("shipping_address.country_code") || "us",
        province: formData.get("shipping_address.province"),
        phone: formData.get("shipping_address.phone"),
      },
      email: formData.get("email"),
    ...
    if (sameAsBilling === "on") {
      ...
      if (billingFirstName) {
        data.billing_address = {
          ...
          last_name: formData.get("billing_address.last_name"),
          address_1: formData.get("billing_address.address_1"),
          address_2: formData.get("billing_address.address_2") || null,
          postal_code: formData.get("billing_address.postal_code"),
          city: formData.get("billing_address.city"),
